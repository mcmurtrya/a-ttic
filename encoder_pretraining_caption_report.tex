...
\begin{abstract}
Visual encoder pretraining leaves a measurable fingerprint on generated
captions. Holding the language model, adaptor, and decoding procedure
fixed, the self-supervised DINOv2 encoder produces \emph{more} projective
spatial language and \emph{longer} captions than the language-supervised
CLIP and SigLIP. The projective spatial direction matches the pre-registered
prediction in \code{methods.md}; the caption-length direction was not
pre-registered. Both effects are consistent in sign across three seeds
and above the pre-registered effect-size floor in every seed (smallest
\(r=0.128\)), but small in absolute size. Pairwise contrasts show CLIP
carries most of the pooled effect: SigLIP sits between CLIP and DINOv2,
so the headline is best read as ``CLIP, and to a lesser extent SigLIP,
differ from DINOv2,'' not as a clean category-level claim. Projective
spatial language is robust across beam and nucleus decoding; caption
length is robust under beam search only. Effects for adjective density
and head-noun specificity appear in single seeds and do not survive
aggregation. The four-encoder experiment uses COCO captions with frozen
GPT-2 medium and a trained prefix adaptor; the cleanest comparable-quality
contrast excludes MAE, which fails both reference-quality preconditions
(CIDEr and SPICE) in all three seeds.
\end{abstract}
...
GPT-2 embedding space. Patch tokens (not the CLS token) are used as the
adaptor input: CLS aggregates information differently across self-supervised
and language-supervised objectives --- it concentrates substantially less
information for DINOv2 and MAE than for CLIP and SigLIP --- and using it
would systematically disadvantage the self-supervised conditions. This
makes the experiment closer to a controlled adaptor-based multimodal probe
than to a fully optimized BLIP-2 or LLaVA-style system.
...
\midrule
CLIP~\cite{radford2021clip} & \code{openai/clip-vit-large-patch14} & image--text &
Contrastive image--text pretraining; representations are explicitly aligned
with natural-language concepts. \\
SigLIP~\cite{zhai2023siglip} & \code{google/siglip-large-patch16-256} & image--text &
...
DINOv2~\cite{oquab2023dinov2} & \code{facebook/dinov2-large} & image-only &
Self-supervised visual consistency / self-distillation; learns semantic visual
structure without captions. \\
MAE~\cite{he2022mae} & \code{facebook/vit-mae-large} & image-only &
...
same images during training. Each adaptor trains with AdamW at learning rate \(1\!\times\!10^{-4}\),
weight decay \(0.01\), and batch size \(256\), with \(500\) warmup steps
followed by a cosine schedule capped at \(35{,}000\) steps. Validation
runs every \(1000\) steps; training stops after five validation evaluations
without improvement. Generation uses beam search with width 5 and nucleus
sampling with \(p=0.9\), both capped at \code{max\_new\_tokens=30}
(see \code{configs/base.yaml}).
...
CIDEr~\cite{vedantam2015cider} (consensus-based n-gram similarity to the COCO
references) and SPICE~\cite{anderson2016spice} (semantic scene-graph overlap
with the references). Both are reported with
...
The secondary family runs all pairwise encoder contrasts, such as CLIP vs.\
SigLIP and CLIP vs.\ DINOv2. This is diagnostic: it checks whether a pooled
...
\paragraph{Deviation from pre-registration.} \code{methods.md} pre-registers a
...
features are less directly usable for caption generation than CLIP, SigLIP,
and DINOv2 under this adaptor-only bottleneck. However, MAE should not be part of
the main comparable-quality style claim.

\paragraph{Independent representational evidence (linear probes).} A
linear-probe analysis on the frozen encoders supports the same conclusion.
Training a one-vs-rest linear classifier on each encoder's pooled patch
features for COCO 80-way object classification (4000 train / 1000 test,
80/20 random split of COCO \code{val2017} --- the same image pool used
for caption generation, but the probe never sees captions) gives mAP
scores 0.73 (CLIP), 0.76 (SigLIP), 0.73 (DINOv2), and 0.53 (MAE); see
\code{captions/probe\_results.csv}. The three
comparable-quality encoders are within 4\% of each other while MAE is
roughly 30\% behind, mirroring the CIDEr and SPICE ordering. The
exclusion of MAE is therefore not an adaptor-specific quirk: MAE's patch
features carry less object-level discriminative signal in the first
place.
...
The mixed-effects table establishes the \emph{direction} of every effect
reported in this paper, since the rank-based \(r\) statistic is unsigned.
It also gives an image-controlled magnitude in each metric's native unit. In the MAE-excluded beam contrast, every robust
...
\([0.262, 0.324]\) for beam projective density,
\([0.185, 0.218]\) for beam caption length, and
\([0.132, 0.188]\) for nucleus projective density --- all comfortably
above the pre-registered 0.10 floor. Two per-seed CIs reach the floor as marginal cases: beam caption length
in seed 3 has CI \([0.099, 0.159]\), with the lower bound fractionally
below 0.10, and nucleus projective density in seed 1 has CI
\([0.077, 0.185]\). In both cases the aggregate is unambiguous and
...
As a final confirmation that the asymptotic Wilcoxon \(p\)-values are not
artifacts of the normal approximation under the zero-inflated metrics, a
10{,}000-iteration paired sign-flip permutation test
(\code{scripts/11\_permutation\_test.py},
\code{captions/permutation\_test.csv}) reproduces the same significance
verdicts as the Wilcoxon BH-corrected \(p\)-values: all robust headline
rows yield \(p_{\text{perm}}\) at the floor of \(1/10{,}001\!\approx\!10^{-4}\),
and the non-robust nucleus caption-length seed-2 row yields
\(p_{\text{perm}}\!=\!0.71\), matching the Wilcoxon \(p_{\text{adj}}\!=\!0.80\).
...
\begin{itemize}
  ...
  under the adaptor-only setup, so style differences involving MAE are
  confounded with quality.
\end{itemize}

\paragraph{Relationship to the pre-registered prediction.}
\code{methods.md} pre-registers a three-part directional prediction: that
the language-supervised encoders, relative to the self-supervised
encoders, will produce captions with (a) higher object-naming density,
(b) higher hypernym depth on head nouns, and (c) lower projective-spatial-term
frequency. The robust headline (projective spatial density: lang-supervised
\(<\) DINOv2) matches part (c) of that prediction in direction; this is the
pre-registered direction, not a surprise. Parts (a) and (b) are not
supported by the data: VG attribute recall and head-noun hypernym depth
both come out with the opposite sign on average (lang-supervised
\emph{lower} than DINOv2), but neither is robust under the cross-seed
aggregation, so the disconfirmation is weak and the right reading is
``not enough evidence to confirm parts (a) or (b),'' not ``parts (a) and
(b) are reversed.''
...
length contrast is essentially CLIP vs.\ DINOv2 (the SigLIP--DINOv2 gap is
...
features are less comparable in this adaptor-only captioning setup and should
...
\begin{enumerate}
  ...
  headline comparison. The \(2\times2\) design was intended to provide
...
size. MAE produces lower CIDEr across seeds under the adaptor-only setup, so
...
Table~\ref{tab:repro} lists the scripts that reproduce every numerical
claim in this report, together with the headline output files each script
emits. Each script is self-contained and reads its inputs from
\code{captions/} and \code{configs/}.

\begin{table}[H]
\centering
\caption{Scripts that reproduce every numerical claim in this report and
the headline output files each emits. ``Phase A'' refers to the seed
2--3 follow-up; the earlier scripts produce the pipeline outputs that
Phase A consumes.}
\label{tab:repro}
...
Path & Role \\
\midrule
\code{scripts/02\_train\_adaptor.py} & Trains one prefix adaptor while freezing the selected visual encoder and GPT-2. Invoked once per encoder (CLIP, SigLIP, DINOv2, MAE) via \code{--encoder}. \\
...
\code{scripts/11\_permutation\_test.py} & 10{,}000-iteration paired sign-flip permutation test. \\
...
bootstrap CI (each seed is resampled independently per iteration --- image
indices are not aligned across seeds --- and the three resampled \(r\)
values are averaged). No BH correction is applied across seeds, so the
aggregate row has no \(p_{\text{adj}}\).
...
Two reading notes. (i)~Two per-seed CIs are marginal at the 0.10
pre-registered floor: beam caption length in seed~3 has lower bound
0.099; nucleus projective density in seed~1 has lower bound 0.077. In both cases the aggregate CI is cleanly above 0.10
