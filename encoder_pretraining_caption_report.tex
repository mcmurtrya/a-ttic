...
the same: the self-supervised encoder (DINOv2) produces \emph{more}
projective spatial language and \emph{longer} captions than the
language-supervised encoders. On projective spatial language this matches
the directional prediction pre-registered in \code{methods.md}; the caption
length direction was not pre-registered. These differences are statistically robust ---
consistent in sign and at or above the pre-registered effect-size floor in
every seed --- but small in absolute terms. Pairwise contrasts show CLIP carries
...
makes the experiment closer to a controlled adapter-based multimodal probe
...
images) gives mAP scores 0.73 (CLIP), 0.76 (SigLIP), 0.73 (DINOv2), and
0.53 (MAE); see \code{captions/probe\_results.csv}. The three
...
exclusion of MAE is therefore not an adapter-specific quirk: MAE's patch
