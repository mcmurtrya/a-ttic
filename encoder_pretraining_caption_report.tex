...
the CIDEr quality precondition. In the CLIP+SigLIP versus DINOv2 contrast,
...
Reference-based caption quality is checked using CIDEr. SPICE was attempted,
but the completed run reports \code{SPICE=n/a} because Java was unavailable in
the execution environment. The same missing dependency caused CIDEr's standard
PTBTokenizer to fail; the run fell back to a regex tokenizer, so the absolute
CIDEr values reported here are not directly comparable to literature numbers.
The quality precondition is a \emph{relative} spread test across encoders,
which remains valid under a consistent fallback tokenizer. Consequently, this
report treats CIDEr as the operative quality precondition and does not make
claims from SPICE.
...
maximum beam CIDEr spread of 10\% of the best encoder. MAE fails this gate in
all three seeds.

\begin{table}[h]
\centering
\caption{Beam CIDEr by seed (regex-tokenizer fallback; see the Metrics section
--- absolute values are not literature-comparable, but the relative spread is).
The precondition fails in every seed because MAE is lower than the other
encoders.}
\label{tab:cider}
\begin{tabular}{@{}lrrrrr@{}}
\toprule
Seed & CLIP & SigLIP & DINOv2 & MAE & Spread fraction \\
\midrule
1 & 1.02 & 1.11 & 1.10 & 0.88 & 0.205 \\
2 & 1.06 & 1.13 & 1.11 & 0.89 & 0.213 \\
3 & 1.03 & 1.14 & 1.11 & 0.88 & 0.230 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
  ...
  \item \textbf{MAE quality confound.} MAE's lower CIDEr means it cannot support
  the same style-level claim as the comparable-quality encoders.
  \item \textbf{SPICE is unavailable.} Java was unavailable in the execution
  environment, so the SPICE scorer could not run. This is a substantive loss:
  SPICE was the only planned reference metric that targets semantic
  proposition overlap rather than n-gram surface similarity, so the report
  cannot make a semantic-quality claim independent of CIDEr.
  \item \textbf{CIDEr tokenizer is degraded.} The same missing Java dependency
  prevented CIDEr's standard PTBTokenizer from running; the scorer fell back
  to a regex tokenizer. Absolute CIDEr values are therefore not
  literature-comparable; only the relative cross-encoder spread, on which the
  quality precondition depends, is used.
...
\code{scripts/08\_caption\_quality.py} & Computes CIDEr quality precondition; SPICE was attempted but unavailable in the completed run. \\
