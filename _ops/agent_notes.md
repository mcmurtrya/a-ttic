...
## INTERVENTION 7 — 2026-05-12 14:28 UTC — killed old clip_rerun_watcher.sh

User authorized killing the stale Intervention-5-era watcher to remove a
conflict risk with Intervention-6's finisher_v2 + clip_reruns_postphase.sh.

- Killed PID 3673783 (`bash /home/ubuntu/clip_rerun_watcher.sh`) at 14:28 UTC.
- The watcher was waiting for "Pipeline complete." in pipeline.out to trigger
  clip_seed1+seed2 reruns directly. With mae_seed3 ~2 h from completion, it
  would have raced finisher_v2/clip_reruns_postphase.sh for GPUs 0+1.
- finisher_v2 (PID 501472) is the sole post-pipeline orchestrator now. Its
  call chain on "Pipeline complete." is:
    clip_reruns_postphase.sh → write clip_reruns.complete → git add/commit/push → shutdown
  (or write clip_reruns.failed and exit non-zero without shutdown on failure).
- Lock file /home/ubuntu/clip_rerun_watcher.lock is fine; kernel releases the
  exclusive flock on process exit. Don't chmod or rm it.
- clip_rerun_watcher.sh itself was NOT deleted (in case rollback is wanted).
  If reviving: be sure to disable finisher_v2's rerun call first, or you'll
  reintroduce the conflict.

## INTERVENTION 8 — 2026-05-12 14:33 UTC — finisher_v2 commit-glob patches

Two finisher_v2.sh edits applied after audit, before the commit/push phase
runs. Syntax-checked (`bash -n`).

1. Captions loop now includes `captions/*.jsonl.orig` so the postphase's
   archived original captions (resume-contaminated clip_seed1) make the
   commit. Previously the `*.jsonl` glob silently dropped them. Reference
   value: a future reviewer can diff old vs new seed1 captions to see the
   effect of resume-contamination.

2. Checkpoint glob now skips `*_orig/` dirs:
     for ck in checkpoints/*/adaptor_best.pt; do
       case "$ck" in *_orig/*) continue ;; esac
       git add -f "$ck"
     done
   Previously the glob would have committed clip_seed{1,2}_orig/adaptor_best.pt
   alongside the new replacements (~48 MB of obsolete adaptors). The total-size
   cap and max-file cap are unchanged; the *_orig dirs still count toward the
   du -sk total (conservative), they just don't get staged.

Net effect at commit time: contaminated-captions reference is preserved in
git; superseded-adaptor weights are NOT in git (still on disk under
checkpoints/clip_seed{1,2}_orig/ until shutdown, then gone).

### Follow-up: orchestration scripts now persisted to _ops/ (14:38 UTC)

Extended finisher_v2's `_ops/` copy block (lines 142-153) to additionally
copy finisher_v2.sh, clip_reruns_postphase.sh, clip_rerun_watcher.sh,
clip_reruns.log, and clip_reruns.complete. User chose "let finisher_v2
handle it" over an immediate commit on the current branch.

Trade-off: if finisher_v2 itself crashes before its commit phase runs,
none of these get persisted. Acceptable risk given the script has
multiple defensive `exit 0` paths that all write a pipeline_summary.txt
